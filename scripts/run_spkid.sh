...
compute_lp() {
    ...
    for filename in $(sort $*); do
        ...
        EXEC="wav2lp 10 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
...
compute_mfcc() {
    ...
    for filename in $(sort $*); do
        mkdir -p `dirname $w/$FEAT/$filename.$FEAT`
        EXEC="wav2mfcc 20 35 0 1 8 25 $db/$filename.wav $w/$FEAT/$filename.$FEAT"
        echo $EXEC && $EXEC || exit 1
    done
}
...
for cmd in $*; do
   ...
   if [[ $cmd == train ]]; then
       ...
       for dir in $db_devel/BLOCK*/SES* ; do
           name=${dir/*\/}
           echo $name ----
           EXEC="gmm_train -v 1 -T 0 -N 20 -m 27 -i 2 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$name.gmm $lists/class/$name.train"
           echo $EXEC && $EXEC || exit 1
           echo
       done
   elif [[ $cmd == test ]]; then
       ...
       EXEC="gmm_train -i0 -v 1 -T 0 -N 20 -m 27 -d $w/$FEAT -e $FEAT -g $w/gmm/$FEAT/$world.gmm $lists/verif/$world.train"
        echo $EXEC && $EXEC || exit 1
